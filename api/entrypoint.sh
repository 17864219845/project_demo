...
if [[ "${MODE}" == "worker" ]]; then
  ...
  exec celery -A app.celery worker -P ${CELERY_WORKER_CLASS:-gevent} $CONCURRENCY_OPTION \
    ...
    --logfile ${CELERY_WORKER_FILE} -Q ${CELERY_QUEUES:-dataset,mail,ops_trace,app_deletion}

elif [[ "${MODE}" == "beat" ]]; then
  exec celery -A app.celery beat --loglevel ${LOG_LEVEL:-INFO} --logfile ${CELERY_BEAT_FILE}
else
